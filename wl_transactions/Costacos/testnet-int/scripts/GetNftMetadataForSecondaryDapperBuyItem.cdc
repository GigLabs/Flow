...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&costacos123_NFT.Collection>(costacos123_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowcostacos123_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = costacos123_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
