...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(costacos123_NFT.CollectionPublicPath)
    .borrow<&{costacos123_NFT.costacos123_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrowcostacos123_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = costacos123_NFT.getSetMetadata(setId: nft!.setId)!
