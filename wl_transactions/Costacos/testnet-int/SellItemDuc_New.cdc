...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let costacos123_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &costacos123_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&costacos123_NFT.Collection>(from: costacos123_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&costacos123_NFT.Collection>(costacos123_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: costacos123_NFT.CollectionPublicPath)
        }

        self.costacos123_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &costacos123_NFT.Collection>(costacos123_NFT.CollectionPublicPath)
