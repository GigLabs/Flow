...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let costacos123_NFTProvider: Capability<&costacos123_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let costacos123_NFTCollectionProviderPrivatePath = /private/costacos123_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&costacos123_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (costacos123_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&costacos123_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (costacos123_NFTCollectionProviderPrivatePath, target: costacos123_NFT.CollectionStoragePath)
        }

        self.costacos123_NFTProvider = acct.getCapability<&costacos123_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(costacos123_NFTCollectionProviderPrivatePath)!
        assert(self.costacos123_NFTProvider.borrow() != nil, message: "Missing or mis-typed costacos123_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@costacos123_NFT.NFT>(){
    ...
    execute {
        ...
        let royaltyRecipient = getAccount(0x05a563afe7729560)
        ...
        self.storefront.createListing(
            nftProviderCapability: self.costacos123_NFTProvider,
            nftType: Type<@costacos123_NFT.NFT>(),
            nftID: saleItemID,
