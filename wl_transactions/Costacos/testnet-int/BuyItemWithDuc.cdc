...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let costacos123_NFTCollection: &costacos123_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&costacos123_NFT.Collection>(from: costacos123_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-costacos123_NFT.createEmptyCollection(nftType: Type<@costacos123_NFT.NFT>()), to: costacos123_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(costacos123_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&costacos123_NFT.Collection>(costacos123_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: costacos123_NFT.CollectionPublicPath)
        }
        ...
        self.costacos123_NFTCollection = buyer.storage.borrow<&costacos123_NFT.Collection>(
            from: costacos123_NFT.CollectionStoragePath
    ...
    execute {
        ...
        self.costacos123_NFTCollection.deposit(token: <-item)
