...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Costacos_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &Costacos_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&Costacos_NFT.Collection>(from: Costacos_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&Costacos_NFT.Collection>(Costacos_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: Costacos_NFT.CollectionPublicPath)
        }

        self.Costacos_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &Costacos_NFT.Collection>(Costacos_NFT.CollectionPublicPath)
