...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&Costacos_NFT.Collection>(Costacos_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowCostacos_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = Costacos_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
