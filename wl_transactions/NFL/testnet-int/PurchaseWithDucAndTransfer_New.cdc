...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{nflInt_NFT.nflInt_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&nflInt_NFT.Collection>(from: nflInt_NFT.CollectionStoragePath) == nil {

      // Create a new empty collection and save it to the account
      buyer.save(<-nflInt_NFT.createEmptyCollection(), to: nflInt_NFT.CollectionStoragePath)

      // Create a public capability to the nflInt_NFT collection
      // that exposes the Collection interface, which now includes
      // the Metadata Resolver to expose Metadata Standard views
      buyer.link<&nflInt_NFT.Collection{NonFungibleToken.CollectionPublic,nflInt_NFT.nflInt_NFTCollectionPublic,MetadataViews.ResolverCollection}>(
  ...
  // Metadata Resolver interface for the Metadata Standard views
  else if !buyer.getCapability<&nflInt_NFT.Collection{NonFungibleToken.CollectionPublic,nflInt_NFT.nflInt_NFTCollectionPublic,MetadataViews.ResolverCollection}>
      (nflInt_NFT.CollectionPublicPath)!.check() { 

      // Unlink the current capability exposing the nflInt_NFT collection,
      // as it needs to be replaced with an updated capability
      buyer.unlink(nflInt_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&nflInt_NFT.Collection>(from: nflInt_NFT.CollectionStoragePath)
    ...
    .getCapability(nflInt_NFT.CollectionPublicPath)!
    .borrow<&{nflInt_NFT.nflInt_NFTCollectionPublic}>()!
  }
  pre {
    // Make sure the seller is the right account
    self.gigAuthAccountAddress == 0x04625c28593d9408 && sellerAddress == 0xa60a22bbd219e76b: "seller must be GigLabs"
  }
  execute {
