...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let nflInt_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &nflInt_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&nflInt_NFT.Collection>(from: nflInt_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&nflInt_NFT.Collection>(nflInt_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: nflInt_NFT.CollectionPublicPath)
        }

        self.nflInt_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &nflInt_NFT.Collection>(nflInt_NFT.CollectionPublicPath)
