...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let nflInt_NFTProvider: Capability<&nflInt_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let nflInt_NFTCollectionProviderPrivatePath = /private/nflInt_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&nflInt_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (nflInt_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&nflInt_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (nflInt_NFTCollectionProviderPrivatePath, target: nflInt_NFT.CollectionStoragePath)
        }

        self.nflInt_NFTProvider = acct.getCapability<&nflInt_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nflInt_NFTCollectionProviderPrivatePath)!
        assert(self.nflInt_NFTProvider.borrow() != nil, message: "Missing or mis-typed nflInt_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@nflInt_NFT.NFT>(){
    ...
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.nflInt_NFTProvider,
            nftType: Type<@nflInt_NFT.NFT>(),
            nftID: saleItemID,
