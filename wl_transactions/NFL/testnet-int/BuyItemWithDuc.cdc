...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let nflInt_NFTCollection: &nflInt_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&nflInt_NFT.Collection>(from: nflInt_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-nflInt_NFT.createEmptyCollection(nftType: Type<@nflInt_NFT.NFT>()), to: nflInt_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(nflInt_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&nflInt_NFT.Collection>(nflInt_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: nflInt_NFT.CollectionPublicPath)
        }
        ...
        self.nflInt_NFTCollection = buyer.storage.borrow<&nflInt_NFT.Collection>(
            from: nflInt_NFT.CollectionStoragePath
    ...
    execute {
        ...
        self.nflInt_NFTCollection.deposit(token: <-item)
