...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(nflInt_NFT.CollectionPublicPath)
    .borrow<&{nflInt_NFT.nflInt_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrownflInt_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = nflInt_NFT.getSetMetadata(setId: nft!.setId)!
