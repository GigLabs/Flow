...
pub fun main(listingResourceID: UInt64, storefrontAddress: Address, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(nflInt_NFT.CollectionPublicPath)
        .borrow<&{nflInt_NFT.nflInt_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrownflInt_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = nflInt_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
