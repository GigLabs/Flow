...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let NFL_NFTProvider: Capability<&NFL_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let NFL_NFTCollectionProviderPrivatePath = /private/NFL_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&NFL_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (NFL_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&NFL_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (NFL_NFTCollectionProviderPrivatePath, target: NFL_NFT.CollectionStoragePath)
        }

        self.NFL_NFTProvider = acct.getCapability<&NFL_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(NFL_NFTCollectionProviderPrivatePath)!
        assert(self.NFL_NFTProvider.borrow() != nil, message: "Missing or mis-typed NFL_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@NFL_NFT.NFT>(){
    ...
    execute {
        ...
        let royaltyRecipient = getAccount(0x2227d6bbba433747)
        ...
        self.storefront.createListing(
            nftProviderCapability: self.NFL_NFTProvider,
            nftType: Type<@NFL_NFT.NFT>(),
            nftID: saleItemID,
