...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let NFL_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &NFL_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&NFL_NFT.Collection>(from: NFL_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&NFL_NFT.Collection>(NFL_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: NFL_NFT.CollectionPublicPath)
        }

        self.NFL_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &NFL_NFT.Collection>(NFL_NFT.CollectionPublicPath)
