...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{NFL_NFT.NFL_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    else if (buyer.getCapability<&NFL_NFT.Collection{NFL_NFT.NFL_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(NFL_NFT.CollectionPublicPath).borrow() == nil) {

        // Unlink the current capability exposing the NFL_NFT collection,
        // as it needs to be replaced with an updated capability
        buyer.unlink(NFL_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&NFL_NFT.Collection>(from: NFL_NFT.CollectionStoragePath)
