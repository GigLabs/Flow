...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let nba_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &nba_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&nba_NFT.Collection>(from: nba_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&nba_NFT.Collection>(nba_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: nba_NFT.CollectionPublicPath)
        }

        self.nba_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &nba_NFT.Collection>(nba_NFT.CollectionPublicPath)
