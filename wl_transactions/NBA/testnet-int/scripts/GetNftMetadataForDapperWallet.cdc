...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(nba_NFT.CollectionPublicPath)
    .borrow<&{nba_NFT.nba_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrownba_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = nba_NFT.getSetMetadata(setId: nft!.setId)!
