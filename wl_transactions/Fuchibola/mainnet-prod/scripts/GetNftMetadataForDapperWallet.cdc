...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(Fuchibola_NFT.CollectionPublicPath)
    .borrow<&{Fuchibola_NFT.Fuchibola_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrowFuchibola_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = Fuchibola_NFT.getSetMetadata(setId: nft!.setId)!
