...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&Fuchibola_NFT.Collection>(from: Fuchibola_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&Fuchibola_NFT.Collection>(Fuchibola_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: Fuchibola_NFT.CollectionPublicPath)
        }

        self.Fuchibola_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &Fuchibola_NFT.Collection>(Fuchibola_NFT.CollectionPublicPath)
