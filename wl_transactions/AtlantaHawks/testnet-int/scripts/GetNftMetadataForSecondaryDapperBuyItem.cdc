...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&atlantahawks_NFT.Collection>(atlantahawks_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowatlantahawks_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = atlantahawks_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
