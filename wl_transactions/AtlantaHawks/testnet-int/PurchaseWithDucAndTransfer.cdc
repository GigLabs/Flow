...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    else if (buyer.getCapability<&atlantahawks_NFT.Collection{atlantahawks_NFT.atlantahawks_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(atlantahawks_NFT.CollectionPublicPath).borrow() == nil) {
  ...
  pre {
    ...
    self.gigAuthAccountAddress == 0x04625c28593d9408 && sellerAddress == 0x922e1605360c7cc7: "seller must be GigLabs"
  }
  execute {
