...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(AtlantaHawks_NFT.CollectionPublicPath)
    .borrow<&{AtlantaHawks_NFT.AtlantaHawks_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrowAtlantaHawks_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = AtlantaHawks_NFT.getSetMetadata(setId: nft!.setId)!
