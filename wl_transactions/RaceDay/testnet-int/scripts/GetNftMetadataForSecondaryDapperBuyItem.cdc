...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&RaceDay_NFT.Collection>(RaceDay_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowRaceDay_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = RaceDay_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
