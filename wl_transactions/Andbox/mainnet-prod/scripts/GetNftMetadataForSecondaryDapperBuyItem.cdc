...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&Andbox_NFT.Collection>(Andbox_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowAndbox_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = Andbox_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
