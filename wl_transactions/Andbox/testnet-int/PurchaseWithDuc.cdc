...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&AndBoxINT_NFT.Collection>(from: AndBoxINT_NFT.CollectionStoragePath) == nil {
        ...
        buyer.link<&AndBoxINT_NFT.Collection{AndBoxINT_NFT.AndBoxINT_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(
    ...
    else if (signer.getCapability<&AndBoxINT_NFT.Collection{AndBoxINT_NFT.AndBoxINT_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(AndBoxINT_NFT.CollectionPublicPath).borrow() == nil) {

        // Unlink the current capability exposing the AndBoxINT_NFT collection,
        // as it needs to be replaced with an updated capability
        buyer.unlink(AndBoxINT_NFT.CollectionPublicPath)
