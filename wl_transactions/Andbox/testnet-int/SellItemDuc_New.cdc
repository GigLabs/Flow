...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let AndBoxINT_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &AndBoxINT_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&AndBoxINT_NFT.Collection>(from: AndBoxINT_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&AndBoxINT_NFT.Collection>(AndBoxINT_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: AndBoxINT_NFT.CollectionPublicPath)
        }

        self.AndBoxINT_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &AndBoxINT_NFT.Collection>(AndBoxINT_NFT.CollectionPublicPath)
    ...
    execute {
        ...
        let royaltyRecipient = getAccount(0xc5d2f97a7a631911)
