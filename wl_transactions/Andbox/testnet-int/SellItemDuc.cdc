...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.AndBoxINT_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>(AndBoxINT_NFT.CollectionStoragePath)
        assert(self.AndBoxINT_NFTProvider.check(), message: "Missing or mis-typed AndBoxINT_NFT.Collection provider")
