...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let AndBoxINT_NFTCollection: &AndBoxINT_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&AndBoxINT_NFT.Collection>(from: AndBoxINT_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-AndBoxINT_NFT.createEmptyCollection(nftType: Type<@AndBoxINT_NFT.NFT>()), to: AndBoxINT_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(AndBoxINT_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&AndBoxINT_NFT.Collection>(AndBoxINT_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: AndBoxINT_NFT.CollectionPublicPath)
        }
        ...
        self.AndBoxINT_NFTCollection = buyer.storage.borrow<&AndBoxINT_NFT.Collection>(
