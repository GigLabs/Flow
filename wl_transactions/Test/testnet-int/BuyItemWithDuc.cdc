...
transaction(listingResourceID: UInt64, storefrontAddress: Address, expectedPrice: UFix64) {
    ...
    let test_NFTCollection: &test_NFT.Collection{NonFungibleToken.Receiver}
    ...
    prepare(dapper: AuthAccount, buyer: AuthAccount) {
        ...
        if buyer.borrow<&test_NFT.Collection>(from: test_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.save(<-test_NFT.createEmptyCollection(), to: test_NFT.CollectionStoragePath)

            // Create a public capability to the test_NFT collection
            // that exposes the Collection interface
            buyer.link<&test_NFT.Collection{NonFungibleToken.CollectionPublic,test_NFT.test_NFTCollectionPublic}>(
        ...
        self.test_NFTCollection = buyer.borrow<&test_NFT.Collection{NonFungibleToken.Receiver}>(
            from: test_NFT.CollectionStoragePath
    ...
    execute {
        ...
        self.test_NFTCollection.deposit(token: <-item)
