...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let test_NFTProvider: Capability<&test_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let test_NFTCollectionProviderPrivatePath = /private/test_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&test_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (test_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&test_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (test_NFTCollectionProviderPrivatePath, target: test_NFT.CollectionStoragePath)
        }

        self.test_NFTProvider = acct.getCapability<&test_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(test_NFTCollectionProviderPrivatePath)!
        assert(self.test_NFTProvider.borrow() != nil, message: "Missing or mis-typed test_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@test_NFT.NFT>(){
    ...
    execute {
        ...
        let royaltyRecipient = getAccount(0x09e580d98d753c9b)
        ...
        self.storefront.createListing(
            nftProviderCapability: self.test_NFTProvider,
            nftType: Type<@test_NFT.NFT>(),
            nftID: saleItemID,
