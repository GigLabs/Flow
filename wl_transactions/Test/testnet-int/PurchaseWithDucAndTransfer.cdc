...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{test_NFT.test_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&test_NFT.Collection>(from: test_NFT.CollectionStoragePath)
    ...
    .getCapability(test_NFT.CollectionPublicPath)!
