...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    else if !buyer.getCapability<&breakingt_NFT.Collection{NonFungibleToken.CollectionPublic,breakingt_NFT.breakingt_NFTCollectionPublic,MetadataViews.ResolverCollection}>
        (breakingt_NFT.CollectionPublicPath)!.check() { 
