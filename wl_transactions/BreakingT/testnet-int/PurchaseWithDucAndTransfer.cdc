...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&breakingt_NFT.Collection>(from: breakingt_NFT.CollectionStoragePath) == nil {
        ...
        buyer.link<&breakingt_NFT.Collection{NonFungibleToken.CollectionPublic,breakingt_NFT.breakingt_NFTCollectionPublic,MetadataViews.ResolverCollection}>(
    ...
    else if (signer.getCapability<&breakingt_NFT.Collection{NonFungibleToken.CollectionPublic,breakingt_NFT.breakingt_NFTCollectionPublic,MetadataViews.ResolverCollection}>(breakingt_NFT.CollectionPublicPath).borrow() == nil) {
  ...
  pre {
    ...
    self.gigAuthAccountAddress == 0x04625c28593d9408 && sellerAddress == 0x36428d95d1ad644d: "seller must be GigLabs"
  }
  execute {
