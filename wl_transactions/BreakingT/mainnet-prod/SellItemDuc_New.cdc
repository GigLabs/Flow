...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let BreakingT_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &BreakingT_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&BreakingT_NFT.Collection>(from: BreakingT_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&BreakingT_NFT.Collection>(BreakingT_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: BreakingT_NFT.CollectionPublicPath)
        }

        self.BreakingT_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &BreakingT_NFT.Collection>(BreakingT_NFT.CollectionPublicPath)
