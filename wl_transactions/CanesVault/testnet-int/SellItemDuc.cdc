...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.Canes_Vault_Int_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &Canes_Vault_Int_NFT.Collection>(Canes_Vault_Int_NFT.CollectionStoragePath)
        assert(self.Canes_Vault_Int_NFTProvider.check(), message: "Missing or mis-typed Canes_Vault_Int_NFT.Collection provider")
