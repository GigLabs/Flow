...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    else if (buyer.getCapability<&Canes_Vault_Int_NFT.Collection{Canes_Vault_Int_NFT.Canes_Vault_Int_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(Canes_Vault_Int_NFT.CollectionPublicPath).borrow() == nil) {
