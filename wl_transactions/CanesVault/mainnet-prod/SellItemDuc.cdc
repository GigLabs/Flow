...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.Canes_Vault_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>(Canes_Vault_NFT.CollectionStoragePath)
        assert(self.Canes_Vault_NFTProvider.check(), message: "Missing or mis-typed Canes_Vault_NFT.Collection provider")
