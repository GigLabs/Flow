...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Canes_Vault_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &Canes_Vault_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&Canes_Vault_NFT.Collection>(from: Canes_Vault_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&Canes_Vault_NFT.Collection>(Canes_Vault_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: Canes_Vault_NFT.CollectionPublicPath)
        }

        self.Canes_Vault_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &Canes_Vault_NFT.Collection>(Canes_Vault_NFT.CollectionPublicPath)
