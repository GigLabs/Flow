...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let giglabs_NFTProvider: Capability<&giglabs_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let giglabs_NFTCollectionProviderPrivatePath = /private/giglabs_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&giglabs_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (giglabs_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&giglabs_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (giglabs_NFTCollectionProviderPrivatePath, target: giglabs_NFT.CollectionStoragePath)
        }

        self.giglabs_NFTProvider = acct.getCapability<&giglabs_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(giglabs_NFTCollectionProviderPrivatePath)!
        assert(self.giglabs_NFTProvider.borrow() != nil, message: "Missing or mis-typed giglabs_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@giglabs_NFT.NFT>(){
    ...
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.giglabs_NFTProvider,
            nftType: Type<@giglabs_NFT.NFT>(),
            nftID: saleItemID,
