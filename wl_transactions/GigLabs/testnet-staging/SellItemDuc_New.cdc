...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let giglabs_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &giglabs_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&giglabs_NFT.Collection>(from: giglabs_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&giglabs_NFT.Collection>(giglabs_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: giglabs_NFT.CollectionPublicPath)
        }

        self.giglabs_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &giglabs_NFT.Collection>(giglabs_NFT.CollectionPublicPath)
