...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{giglabs_NFT.giglabs_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&giglabs_NFT.Collection>(from: giglabs_NFT.CollectionStoragePath) == nil {
        ...
        buyer.link<&giglabs_NFT.Collection{giglabs_NFT.giglabs_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(
    ...
    else if (buyer.getCapability<&giglabs_NFT.Collection{giglabs_NFT.giglabs_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(giglabs_NFT.CollectionPublicPath).borrow() == nil) {

        // Unlink the current capability exposing the giglabs_NFT collection,
        // as it needs to be replaced with an updated capability
        buyer.unlink(giglabs_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&giglabs_NFT.Collection>(from: giglabs_NFT.CollectionStoragePath)
    ...
    .getCapability(giglabs_NFT.CollectionPublicPath)!
    .borrow<&{giglabs_NFT.giglabs_NFTCollectionPublic}>()!
  }
  pre {
