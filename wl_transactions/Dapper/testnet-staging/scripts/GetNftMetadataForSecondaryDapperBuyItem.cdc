...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(Dapper_NFT.CollectionPublicPath)
        .borrow<&{Dapper_NFT.Dapper_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowDapper_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = Dapper_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
