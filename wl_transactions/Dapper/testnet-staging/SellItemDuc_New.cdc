...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Dapper_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &Dapper_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&Dapper_NFT.Collection>(from: Dapper_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&Dapper_NFT.Collection>(Dapper_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: Dapper_NFT.CollectionPublicPath)
        }

        self.Dapper_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &Dapper_NFT.Collection>(Dapper_NFT.CollectionPublicPath)
