...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Dapper_NFTProvider: Capability<&Dapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let Dapper_NFTCollectionProviderPrivatePath = /private/Dapper_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&Dapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (Dapper_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&Dapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (Dapper_NFTCollectionProviderPrivatePath, target: Dapper_NFT.CollectionStoragePath)
        }

        self.Dapper_NFTProvider = acct.getCapability<&Dapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(Dapper_NFTCollectionProviderPrivatePath)!
        assert(self.Dapper_NFTProvider.borrow() != nil, message: "Missing or mis-typed Dapper_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@Dapper_NFT.NFT>(){
    ...
    pre {
        self.gigAddress == 0x36b754ce392af85b: "Requires valid authorizing signature"
    }
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.Dapper_NFTProvider,
            nftType: Type<@Dapper_NFT.NFT>(),
            nftID: saleItemID,
