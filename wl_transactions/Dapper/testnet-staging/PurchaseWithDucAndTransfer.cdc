...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{Dapper_NFT.Dapper_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&Dapper_NFT.Collection>(from: Dapper_NFT.CollectionStoragePath) == nil {

        // Create a new empty collection and save it to the account
        buyer.save(<-Dapper_NFT.createEmptyCollection(), to: Dapper_NFT.CollectionStoragePath)

        // Create a public capability to the Dapper_NFT collection
        // that exposes the Collection interface, which now includes
        // the Metadata Resolver to expose Metadata Standard views
        buyer.link<&Dapper_NFT.Collection{Dapper_NFT.Dapper_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(
    ...
    else if (buyer.getCapability<&Dapper_NFT.Collection{Dapper_NFT.Dapper_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(Dapper_NFT.CollectionPublicPath).borrow() == nil) {

        // Unlink the current capability exposing the Dapper_NFT collection,
        // as it needs to be replaced with an updated capability
        buyer.unlink(Dapper_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&Dapper_NFT.Collection>(from: Dapper_NFT.CollectionStoragePath)
    ...
    .getCapability(Dapper_NFT.CollectionPublicPath)!
    .borrow<&{Dapper_NFT.Dapper_NFTCollectionPublic}>()!
  }
  pre {
    // Make sure the seller is the right account
    self.gigAuthAccountAddress == 0x36b754ce392af85b && sellerAddress == 0xa60a22bbd219e76b: "seller must be GigLabs"
  }
  execute {
