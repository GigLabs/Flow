...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let Dapper_NFTCollection: &Dapper_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&Dapper_NFT.Collection>(from: Dapper_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-Dapper_NFT.createEmptyCollection(nftType: Type<@Dapper_NFT.NFT>()), to: Dapper_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(Dapper_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&Dapper_NFT.Collection>(Dapper_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: Dapper_NFT.CollectionPublicPath)
        }
        ...
        self.Dapper_NFTCollection = buyer.storage.borrow<&Dapper_NFT.Collection>(
            from: Dapper_NFT.CollectionStoragePath
    ...
    execute {
        ...
        self.Dapper_NFTCollection.deposit(token: <-item)
