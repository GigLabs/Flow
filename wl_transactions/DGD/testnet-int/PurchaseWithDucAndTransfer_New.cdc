...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{dgd_NFT.dgd_NFTCollectionPublic}
  ...
  // Metadata Resolver interface for the Metadata Standard views
  else if !buyer.getCapability<&dgd_NFT.Collection{NonFungibleToken.CollectionPublic,dgd_NFT.dgd_NFTCollectionPublic,MetadataViews.ResolverCollection}>
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&dgd_NFT.Collection>(from: dgd_NFT.CollectionStoragePath)
