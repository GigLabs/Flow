...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let dgd_NFTProvider: Capability<&dgd_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let dgd_NFTCollectionProviderPrivatePath = /private/dgd_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&dgd_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (dgd_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&dgd_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (dgd_NFTCollectionProviderPrivatePath, target: dgd_NFT.CollectionStoragePath)
        }

        self.dgd_NFTProvider = acct.getCapability<&dgd_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(dgd_NFTCollectionProviderPrivatePath)!
