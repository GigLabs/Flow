...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&DGD_NFT.Collection>(DGD_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowDGD_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = DGD_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
