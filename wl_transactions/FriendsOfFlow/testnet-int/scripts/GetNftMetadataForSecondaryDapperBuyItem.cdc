...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&friendsOfFlow_NFT.Collection>(friendsOfFlow_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowfriendsOfFlow_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = friendsOfFlow_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
