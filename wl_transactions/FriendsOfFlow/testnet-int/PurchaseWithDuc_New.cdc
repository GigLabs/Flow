...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    else if !buyer.getCapability<&friendsOfFlow_NFT.Collection{NonFungibleToken.CollectionPublic,friendsOfFlow_NFT.friendsOfFlow_NFTCollectionPublic,MetadataViews.ResolverCollection}>
