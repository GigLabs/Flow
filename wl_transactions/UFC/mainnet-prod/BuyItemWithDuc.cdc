...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let UFC_NFTCollection: &UFC_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&UFC_NFT.Collection>(from: UFC_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-UFC_NFT.createEmptyCollection(nftType: Type<@UFC_NFT.NFT>()), to: UFC_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(UFC_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&UFC_NFT.Collection>(UFC_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: UFC_NFT.CollectionPublicPath)
        }
        ...
        self.UFC_NFTCollection = buyer.storage.borrow<&UFC_NFT.Collection>(
            from: UFC_NFT.CollectionStoragePath
    ...
    execute {
        ...
        self.UFC_NFTCollection.deposit(token: <-item)
