...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let UFC_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &UFC_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&UFC_NFT.Collection>(from: UFC_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&UFC_NFT.Collection>(UFC_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: UFC_NFT.CollectionPublicPath)
        }

        self.UFC_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &UFC_NFT.Collection>(UFC_NFT.CollectionPublicPath)
