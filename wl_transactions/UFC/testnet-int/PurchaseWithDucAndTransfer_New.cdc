...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&ufcInt_NFT.Collection>(from: ufcInt_NFT.CollectionStoragePath) == nil {
      ...
      buyer.link<&ufcInt_NFT.Collection{NonFungibleToken.CollectionPublic,ufcInt_NFT.ufcInt_NFTCollectionPublic,MetadataViews.ResolverCollection}>(
  ...
  else if !buyer.getCapability<&ufcInt_NFT.Collection{NonFungibleToken.CollectionPublic,ufcInt_NFT.ufcInt_NFTCollectionPublic,MetadataViews.ResolverCollection}>
      (ufcInt_NFT.CollectionPublicPath)!.check() { 
