...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{ufcInt_NFT.ufcInt_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&ufcInt_NFT.Collection>(from: ufcInt_NFT.CollectionStoragePath) == nil {

        // Create a new empty collection and save it to the account
        buyer.save(<-ufcInt_NFT.createEmptyCollection(), to: ufcInt_NFT.CollectionStoragePath)

        // Create a public capability to the ufcInt_NFT collection
        // that exposes the Collection interface, which now includes
        // the Metadata Resolver to expose Metadata Standard views
        buyer.link<&ufcInt_NFT.Collection{ufcInt_NFT.ufcInt_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(
    ...
    else if (signer.getCapability<&ufcInt_NFT.Collection{ufcInt_NFT.ufcInt_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(ufcInt_NFT.CollectionPublicPath).borrow() == nil) {

        // Unlink the current capability exposing the ufcInt_NFT collection,
        // as it needs to be replaced with an updated capability
        buyer.unlink(ufcInt_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&ufcInt_NFT.Collection>(from: ufcInt_NFT.CollectionStoragePath)
    ...
    .getCapability(ufcInt_NFT.CollectionPublicPath)!
    .borrow<&{ufcInt_NFT.ufcInt_NFTCollectionPublic}>()!
  }
  pre {
