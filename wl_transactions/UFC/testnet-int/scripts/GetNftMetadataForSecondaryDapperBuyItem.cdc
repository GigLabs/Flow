...
pub fun main(listingResourceID: UInt64, storefrontAddress: Address, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(ufcInt_NFT.CollectionPublicPath)
        .borrow<&{ufcInt_NFT.ufcInt_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowufcInt_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = ufcInt_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
