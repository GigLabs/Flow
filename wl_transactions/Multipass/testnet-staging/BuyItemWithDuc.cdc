...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let Multipass_NFTCollection: &Multipass_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&Multipass_NFT.Collection>(from: Multipass_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-Multipass_NFT.createEmptyCollection(nftType: Type<@Multipass_NFT.NFT>()), to: Multipass_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(Multipass_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&Multipass_NFT.Collection>(Multipass_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: Multipass_NFT.CollectionPublicPath)
        }
        ...
        self.Multipass_NFTCollection = buyer.storage.borrow<&Multipass_NFT.Collection>(
