...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&Multipass_NFT.Collection>(from: Multipass_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&Multipass_NFT.Collection>(Multipass_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: Multipass_NFT.CollectionPublicPath)
        }

        self.Multipass_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &Multipass_NFT.Collection>(Multipass_NFT.CollectionPublicPath)
