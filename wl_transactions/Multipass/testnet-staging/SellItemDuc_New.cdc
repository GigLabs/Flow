...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Multipass_NFTProvider: Capability<&Multipass_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let Multipass_NFTCollectionProviderPrivatePath = /private/Multipass_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&Multipass_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (Multipass_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&Multipass_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (Multipass_NFTCollectionProviderPrivatePath, target: Multipass_NFT.CollectionStoragePath)
        }

        self.Multipass_NFTProvider = acct.getCapability<&Multipass_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(Multipass_NFTCollectionProviderPrivatePath)!
        assert(self.Multipass_NFTProvider.borrow() != nil, message: "Missing or mis-typed Multipass_NFT.Collection provider")
    ...
    pre {
        self.gigAddress == 0xedd8d5484a85a86c: "Requires valid authorizing signature"
    }
    execute {
