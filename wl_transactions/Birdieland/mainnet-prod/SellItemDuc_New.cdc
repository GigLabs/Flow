...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Birdieland_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &Birdieland_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&Birdieland_NFT.Collection>(from: Birdieland_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&Birdieland_NFT.Collection>(Birdieland_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: Birdieland_NFT.CollectionPublicPath)
        }

        self.Birdieland_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &Birdieland_NFT.Collection>(Birdieland_NFT.CollectionPublicPath)
    ...
    pre {
        self.gigAddress == 0x59e3d094592231a7: "Requires valid authorizing signature"
    }
    execute {
