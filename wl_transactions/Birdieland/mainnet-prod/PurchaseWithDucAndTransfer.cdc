...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{Birdieland_NFT.Birdieland_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    else if (buyer.getCapability<&Birdieland_NFT.Collection{Birdieland_NFT.Birdieland_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(Birdieland_NFT.CollectionPublicPath).borrow() == nil) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&Birdieland_NFT.Collection>(from: Birdieland_NFT.CollectionStoragePath)
