...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let izon_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &izon_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&izon_NFT.Collection>(from: izon_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&izon_NFT.Collection>(izon_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: izon_NFT.CollectionPublicPath)
        }

        self.izon_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &izon_NFT.Collection>(izon_NFT.CollectionPublicPath)
