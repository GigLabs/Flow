...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(izon_NFT.CollectionPublicPath)
        .borrow<&{izon_NFT.izon_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowizon_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = izon_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
