...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let izon_NFTProvider: Capability<&izon_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let izon_NFTCollectionProviderPrivatePath = /private/izon_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&izon_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (izon_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&izon_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (izon_NFTCollectionProviderPrivatePath, target: izon_NFT.CollectionStoragePath)
        }

        self.izon_NFTProvider = acct.getCapability<&izon_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(izon_NFTCollectionProviderPrivatePath)!
        assert(self.izon_NFTProvider.borrow() != nil, message: "Missing or mis-typed izon_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@izon_NFT.NFT>(){
    ...
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.izon_NFTProvider,
            nftType: Type<@izon_NFT.NFT>(),
            nftID: saleItemID,
