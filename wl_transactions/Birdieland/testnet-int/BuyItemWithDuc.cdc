...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let izon_NFTCollection: &izon_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&izon_NFT.Collection>(from: izon_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-izon_NFT.createEmptyCollection(nftType: Type<@izon_NFT.NFT>()), to: izon_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(izon_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&izon_NFT.Collection>(izon_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: izon_NFT.CollectionPublicPath)
        }
        ...
        self.izon_NFTCollection = buyer.storage.borrow<&izon_NFT.Collection>(
            from: izon_NFT.CollectionStoragePath
    ...
    execute {
        ...
        self.izon_NFTCollection.deposit(token: <-item)
