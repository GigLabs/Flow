...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{izon_NFT.izon_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&izon_NFT.Collection>(from: izon_NFT.CollectionStoragePath) == nil {
        ...
        buyer.link<&izon_NFT.Collection{NonFungibleToken.CollectionPublic,izon_NFT.izon_NFTCollectionPublic,MetadataViews.ResolverCollection}>(
    ...
    else if (signer.getCapability<&izon_NFT.Collection{NonFungibleToken.CollectionPublic,izon_NFT.izon_NFTCollectionPublic,MetadataViews.ResolverCollection}>(izon_NFT.CollectionPublicPath).borrow() == nil) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&izon_NFT.Collection>(from: izon_NFT.CollectionStoragePath)
    ...
    .getCapability(izon_NFT.CollectionPublicPath)!
    .borrow<&{izon_NFT.izon_NFTCollectionPublic}>()!
  }
  pre {
