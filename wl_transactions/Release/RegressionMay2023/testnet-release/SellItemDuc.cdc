...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.regressionmay2023_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &regressionmay2023_NFT.Collection>(regressionmay2023_NFT.CollectionStoragePath)
        assert(self.regressionmay2023_NFTProvider.check(), message: "Missing or mis-typed regressionmay2023_NFT.Collection provider")
