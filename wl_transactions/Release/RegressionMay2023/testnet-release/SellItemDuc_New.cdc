...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let regressionmay2023_NFTProvider: Capability<&regressionmay2023_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let regressionmay2023_NFTCollectionProviderPrivatePath = /private/regressionmay2023_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&regressionmay2023_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (regressionmay2023_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&regressionmay2023_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (regressionmay2023_NFTCollectionProviderPrivatePath, target: regressionmay2023_NFT.CollectionStoragePath)
        }

        self.regressionmay2023_NFTProvider = acct.getCapability<&regressionmay2023_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(regressionmay2023_NFTCollectionProviderPrivatePath)!
