...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(dapper2023apr_NFT.CollectionPublicPath)
        .borrow<&{dapper2023apr_NFT.dapper2023apr_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowdapper2023apr_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = dapper2023apr_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
