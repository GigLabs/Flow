...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.dapper2023apr_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &dapper2023apr_NFT.Collection>(dapper2023apr_NFT.CollectionStoragePath)
        assert(self.dapper2023apr_NFTProvider.check(), message: "Missing or mis-typed dapper2023apr_NFT.Collection provider")
