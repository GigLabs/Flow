...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(dapperrelease2point0_NFT.CollectionPublicPath)
        .borrow<&{dapperrelease2point0_NFT.dapperrelease2point0_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowdapperrelease2point0_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = dapperrelease2point0_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
