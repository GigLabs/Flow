...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let dapperrelease2point0_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &dapperrelease2point0_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&dapperrelease2point0_NFT.Collection>(from: dapperrelease2point0_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&dapperrelease2point0_NFT.Collection>(dapperrelease2point0_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: dapperrelease2point0_NFT.CollectionPublicPath)
        }

        self.dapperrelease2point0_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &dapperrelease2point0_NFT.Collection>(dapperrelease2point0_NFT.CollectionPublicPath)
    ...
    pre {
        self.gigAddress == 0xd0c7d5711cb0dc51: "Requires valid authorizing signature"
    }
    execute {
