...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{DapPen_NFT.DapPen_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&DapPen_NFT.Collection>(from: DapPen_NFT.CollectionStoragePath) == nil {

        // Create a new empty collection and save it to the account
        buyer.save(<-DapPen_NFT.createEmptyCollection(), to: DapPen_NFT.CollectionStoragePath)

        // Create a public capability to the DapPen_NFT collection
        // that exposes the Collection interface, which now includes
        // the Metadata Resolver to expose Metadata Standard views
        buyer.link<&DapPen_NFT.Collection{DapPen_NFT.DapPen_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(
    ...
    else if (buyer.getCapability<&DapPen_NFT.Collection{DapPen_NFT.DapPen_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(DapPen_NFT.CollectionPublicPath).borrow() == nil) {

        // Unlink the current capability exposing the DapPen_NFT collection,
        // as it needs to be replaced with an updated capability
        buyer.unlink(DapPen_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&DapPen_NFT.Collection>(from: DapPen_NFT.CollectionStoragePath)
    ...
    .getCapability(DapPen_NFT.CollectionPublicPath)!
    .borrow<&{DapPen_NFT.DapPen_NFTCollectionPublic}>()!
  }
  pre {
