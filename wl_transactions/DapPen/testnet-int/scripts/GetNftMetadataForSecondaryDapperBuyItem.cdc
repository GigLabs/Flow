...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(DapPen_NFT.CollectionPublicPath)
        .borrow<&{DapPen_NFT.DapPen_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowDapPen_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = DapPen_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
