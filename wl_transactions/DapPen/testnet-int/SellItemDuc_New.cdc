...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&DapPen_NFT.Collection>(from: DapPen_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&DapPen_NFT.Collection>(DapPen_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: DapPen_NFT.CollectionPublicPath)
        }

        self.DapPen_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &DapPen_NFT.Collection>(DapPen_NFT.CollectionPublicPath)
