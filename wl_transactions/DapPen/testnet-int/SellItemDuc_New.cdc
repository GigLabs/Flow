...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let DapPen_NFTProvider: Capability<&DapPen_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let DapPen_NFTCollectionProviderPrivatePath = /private/DapPen_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&DapPen_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (DapPen_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&DapPen_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (DapPen_NFTCollectionProviderPrivatePath, target: DapPen_NFT.CollectionStoragePath)
        }

        self.DapPen_NFTProvider = acct.getCapability<&DapPen_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(DapPen_NFTCollectionProviderPrivatePath)!
        assert(self.DapPen_NFTProvider.borrow() != nil, message: "Missing or mis-typed DapPen_NFT.Collection provider")
    ...
    pre {
        self.gigAddress == 0x1df9c41532276279: "Requires valid authorizing signature"
    }
    execute {
