...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&toddlocal_NFT.Collection>(from: toddlocal_NFT.CollectionStoragePath) == nil {
      ...
      buyer.link<&toddlocal_NFT.Collection{NonFungibleToken.CollectionPublic,toddlocal_NFT.toddlocal_NFTCollectionPublic,MetadataViews.ResolverCollection}>(
  ...
  else if !buyer.getCapability<&toddlocal_NFT.Collection{NonFungibleToken.CollectionPublic,toddlocal_NFT.toddlocal_NFTCollectionPublic,MetadataViews.ResolverCollection}>
      (toddlocal_NFT.CollectionPublicPath)!.check() { 

      // Unlink the current capability exposing the toddlocal_NFT collection,
      // as it needs to be replaced with an updated capability
      buyer.unlink(toddlocal_NFT.CollectionPublicPath)
  ...
  pre {
    ...
    self.gigAuthAccountAddress == 0xf3e8f8ae2e9e2fec && sellerAddress == 0x074bae238bc6b419: "seller must be GigLabs"
  }
  execute {
