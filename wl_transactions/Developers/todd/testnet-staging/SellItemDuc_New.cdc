...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&ToddDapper_NFT.Collection>(from: ToddDapper_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&ToddDapper_NFT.Collection>(ToddDapper_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: ToddDapper_NFT.CollectionPublicPath)
        }

        self.ToddDapper_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &ToddDapper_NFT.Collection>(ToddDapper_NFT.CollectionPublicPath)
    ...
    pre {
        self.gigAddress == 0x074bae238bc6b419: "Requires valid authorizing signature"
    }
    execute {
