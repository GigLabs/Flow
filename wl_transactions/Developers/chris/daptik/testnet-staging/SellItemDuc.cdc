...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.daptik_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &daptik_NFT.Collection>(daptik_NFT.CollectionStoragePath)
        assert(self.daptik_NFTProvider.check(), message: "Missing or mis-typed daptik_NFT.Collection provider")
