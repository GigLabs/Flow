...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.testify_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &testify_NFT.Collection>(testify_NFT.CollectionStoragePath)
        assert(self.testify_NFTProvider.check(), message: "Missing or mis-typed testify_NFT.Collection provider")
