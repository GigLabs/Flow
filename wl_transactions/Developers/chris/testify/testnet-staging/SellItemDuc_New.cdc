...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let testify_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &testify_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&testify_NFT.Collection>(from: testify_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&testify_NFT.Collection>(testify_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: testify_NFT.CollectionPublicPath)
        }

        self.testify_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &testify_NFT.Collection>(testify_NFT.CollectionPublicPath)
        assert(self.testify_NFTProvider.borrow() != nil, message: "Missing or mis-typed testify_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@testify_NFT.NFT>(){
    ...
    pre {
        self.gigAddress == 0x36aa48b0c24b8897: "Requires valid authorizing signature"
    }
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.testify_NFTProvider,
            nftType: Type<@testify_NFT.NFT>(),
            nftID: saleItemID,
