...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(ryandapper_NFT.CollectionPublicPath)
        .borrow<&{ryandapper_NFT.ryandapper_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowryandapper_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = ryandapper_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
