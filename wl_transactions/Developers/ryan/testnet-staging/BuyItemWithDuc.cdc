...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let ryandapper_NFTCollection: &ryandapper_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&ryandapper_NFT.Collection>(from: ryandapper_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-ryandapper_NFT.createEmptyCollection(nftType: Type<@ryandapper_NFT.NFT>()), to: ryandapper_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(ryandapper_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&ryandapper_NFT.Collection>(ryandapper_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: ryandapper_NFT.CollectionPublicPath)
        }
        ...
        self.ryandapper_NFTCollection = buyer.storage.borrow<&ryandapper_NFT.Collection>(
            from: ryandapper_NFT.CollectionStoragePath
    ...
    execute {
        ...
        self.ryandapper_NFTCollection.deposit(token: <-item)
