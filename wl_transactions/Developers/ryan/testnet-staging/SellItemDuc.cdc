...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let ryandapper_NFTProvider: Capability<&ryandapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let ryandapper_NFTCollectionProviderPrivatePath = /private/ryandapper_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&ryandapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (ryandapper_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&ryandapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (ryandapper_NFTCollectionProviderPrivatePath, target: ryandapper_NFT.CollectionStoragePath)
        }

        self.ryandapper_NFTProvider = acct.getCapability<&ryandapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(ryandapper_NFTCollectionProviderPrivatePath)!
        assert(self.ryandapper_NFTProvider.borrow() != nil, message: "Missing or mis-typed ryandapper_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@ryandapper_NFT.NFT>(){
    ...
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.ryandapper_NFTProvider,
            nftType: Type<@ryandapper_NFT.NFT>(),
            nftID: saleItemID,
