...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{ryandapper_NFT.ryandapper_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&ryandapper_NFT.Collection>(from: ryandapper_NFT.CollectionStoragePath)
    ...
    .getCapability(ryandapper_NFT.CollectionPublicPath)!
    .borrow<&{ryandapper_NFT.ryandapper_NFTCollectionPublic}>()!
  }
  pre {
