...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let drewdapper_NFTCollection: &drewdapper_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&drewdapper_NFT.Collection>(from: drewdapper_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-drewdapper_NFT.createEmptyCollection(nftType: Type<@drewdapper_NFT.NFT>()), to: drewdapper_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(drewdapper_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&drewdapper_NFT.Collection>(drewdapper_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: drewdapper_NFT.CollectionPublicPath)
        }
        ...
        self.drewdapper_NFTCollection = buyer.storage.borrow<&drewdapper_NFT.Collection>(
