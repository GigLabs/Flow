...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let drewdapper_NFTProvider: Capability<&drewdapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let drewdapper_NFTCollectionProviderPrivatePath = /private/drewdapper_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&drewdapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (drewdapper_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&drewdapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (drewdapper_NFTCollectionProviderPrivatePath, target: drewdapper_NFT.CollectionStoragePath)
        }

        self.drewdapper_NFTProvider = acct.getCapability<&drewdapper_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(drewdapper_NFTCollectionProviderPrivatePath)!
