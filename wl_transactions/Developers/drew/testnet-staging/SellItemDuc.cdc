...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.drewdapper_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>(drewdapper_NFT.CollectionStoragePath)
        assert(self.drewdapper_NFTProvider.check(), message: "Missing or mis-typed drewdapper_NFT.Collection provider")
