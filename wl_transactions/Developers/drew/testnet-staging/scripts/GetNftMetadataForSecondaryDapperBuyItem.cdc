...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&drewdapper_NFT.Collection>(drewdapper_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowdrewdapper_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = drewdapper_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
