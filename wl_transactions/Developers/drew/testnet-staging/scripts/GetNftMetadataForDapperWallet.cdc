...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(drewdapper_NFT.CollectionPublicPath)
    .borrow<&{drewdapper_NFT.drewdapper_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrowdrewdapper_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = drewdapper_NFT.getSetMetadata(setId: nft!.setId)!
