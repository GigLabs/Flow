...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&podunks23_NFT.Collection>(podunks23_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowpodunks23_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = podunks23_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
