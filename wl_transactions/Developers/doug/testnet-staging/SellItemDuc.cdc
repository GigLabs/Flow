...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.podunks23_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &podunks23_NFT.Collection>(podunks23_NFT.CollectionStoragePath)
        assert(self.podunks23_NFTProvider.check(), message: "Missing or mis-typed podunks23_NFT.Collection provider")
