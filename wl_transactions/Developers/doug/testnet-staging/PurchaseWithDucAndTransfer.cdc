...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{podunks23_NFT.podunks23_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&podunks23_NFT.Collection>(from: podunks23_NFT.CollectionStoragePath)
    ...
    .getCapability(podunks23_NFT.CollectionPublicPath)!
    .borrow<&{podunks23_NFT.podunks23_NFTCollectionPublic}>()!
  }
  pre {
