...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{releasejan2023_NFT.releasejan2023_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&releasejan2023_NFT.Collection>(from: releasejan2023_NFT.CollectionStoragePath)
    ...
    .getCapability(releasejan2023_NFT.CollectionPublicPath)!
    .borrow<&{releasejan2023_NFT.releasejan2023_NFTCollectionPublic}>()!
  }
  pre {
    // Make sure the seller is the right account
    self.gigAuthAccountAddress == 0x5d2efb448f701c35 && sellerAddress == 0x5d2efb448f701c35: "seller must be GigLabs"
  }
  execute {
