...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.releasejan2023_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &releasejan2023_NFT.Collection>(releasejan2023_NFT.CollectionStoragePath)
        assert(self.releasejan2023_NFTProvider.check(), message: "Missing or mis-typed releasejan2023_NFT.Collection provider")
