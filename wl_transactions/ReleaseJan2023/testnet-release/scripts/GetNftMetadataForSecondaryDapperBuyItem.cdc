...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(releasejan2023_NFT.CollectionPublicPath)
        .borrow<&{releasejan2023_NFT.releasejan2023_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowreleasejan2023_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = releasejan2023_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
