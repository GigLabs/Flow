...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Cimelio_NFTProvider: Capability<&Cimelio_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let Cimelio_NFTCollectionProviderPrivatePath = /private/Cimelio_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&Cimelio_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (Cimelio_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&Cimelio_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (Cimelio_NFTCollectionProviderPrivatePath, target: Cimelio_NFT.CollectionStoragePath)
        }

        self.Cimelio_NFTProvider = acct.getCapability<&Cimelio_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(Cimelio_NFTCollectionProviderPrivatePath)!
        assert(self.Cimelio_NFTProvider.borrow() != nil, message: "Missing or mis-typed Cimelio_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@Cimelio_NFT.NFT>(){
    ...
    pre {
        self.gigAddress == 0x2c9de937c319468d: "Requires valid authorizing signature"
    }
    execute {
        ...
        let royaltyRecipient = getAccount(0x56b610f9c0dbf02b)
        ...
        self.storefront.createListing(
            nftProviderCapability: self.Cimelio_NFTProvider,
