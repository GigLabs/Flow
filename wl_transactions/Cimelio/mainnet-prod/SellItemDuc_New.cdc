...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let Cimelio_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &Cimelio_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&Cimelio_NFT.Collection>(from: Cimelio_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&Cimelio_NFT.Collection>(Cimelio_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: Cimelio_NFT.CollectionPublicPath)
        }

        self.Cimelio_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &Cimelio_NFT.Collection>(Cimelio_NFT.CollectionPublicPath)
