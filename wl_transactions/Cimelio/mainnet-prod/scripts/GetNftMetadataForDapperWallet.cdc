...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(Cimelio_NFT.CollectionPublicPath)
    .borrow<&{Cimelio_NFT.Cimelio_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrowCimelio_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = Cimelio_NFT.getSetMetadata(setId: nft!.setId)!
