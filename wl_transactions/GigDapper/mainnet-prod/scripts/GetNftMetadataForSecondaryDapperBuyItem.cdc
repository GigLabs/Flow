...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(GigDapper_NFT.CollectionPublicPath)
        .borrow<&{GigDapper_NFT.GigDapper_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowGigDapper_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = GigDapper_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
