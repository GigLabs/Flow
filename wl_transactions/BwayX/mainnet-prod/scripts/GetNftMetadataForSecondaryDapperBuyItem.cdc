...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(BWAYX_NFT.CollectionPublicPath)
        .borrow<&{BWAYX_NFT.BWAYX_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowBWAYX_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = BWAYX_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
