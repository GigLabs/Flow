...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let BWAYX_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &BWAYX_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&BWAYX_NFT.Collection>(from: BWAYX_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&BWAYX_NFT.Collection>(BWAYX_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: BWAYX_NFT.CollectionPublicPath)
        }

        self.BWAYX_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &BWAYX_NFT.Collection>(BWAYX_NFT.CollectionPublicPath)
    ...
    pre {
        self.gigAddress == 0xf02b15e11eb3715b: "Requires valid authorizing signature"
    }
    execute {
