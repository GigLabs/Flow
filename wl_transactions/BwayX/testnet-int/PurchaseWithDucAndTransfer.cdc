...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{bwayx_NFT.bwayx_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&bwayx_NFT.Collection>(from: bwayx_NFT.CollectionStoragePath)
    ...
    .borrow<&{bwayx_NFT.bwayx_NFTCollectionPublic}>()!
  }
  pre {
    // Make sure the seller is the right account
    self.gigAuthAccountAddress == 0x477638bc70a9341e && sellerAddress == 0x6f8aa41eedff1158: "seller must be GigLabs"
  }
  execute {
