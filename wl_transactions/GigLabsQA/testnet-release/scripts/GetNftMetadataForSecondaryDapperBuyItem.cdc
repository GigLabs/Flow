...
pub fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.getCapability(GigLabsQA_NFT.CollectionPublicPath)
        .borrow<&{GigLabsQA_NFT.GigLabsQA_NFTCollectionPublic}>()
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowGigLabsQA_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = GigLabsQA_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
