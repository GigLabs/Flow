...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let GigLabsQA_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &GigLabsQA_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&GigLabsQA_NFT.Collection>(from: GigLabsQA_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&GigLabsQA_NFT.Collection>(GigLabsQA_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: GigLabsQA_NFT.CollectionPublicPath)
        }

        self.GigLabsQA_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &GigLabsQA_NFT.Collection>(GigLabsQA_NFT.CollectionPublicPath)
