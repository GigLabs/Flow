...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.GigLabsQA_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>(GigLabsQA_NFT.CollectionStoragePath)
        assert(self.GigLabsQA_NFTProvider.check(), message: "Missing or mis-typed GigLabsQA_NFT.Collection provider")
