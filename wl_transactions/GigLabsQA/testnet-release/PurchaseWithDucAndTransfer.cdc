...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{GigLabsQA_NFT.GigLabsQA_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&GigLabsQA_NFT.Collection>(from: GigLabsQA_NFT.CollectionStoragePath) == nil {
        ...
        buyer.link<&GigLabsQA_NFT.Collection{NonFungibleToken.CollectionPublic,GigLabsQA_NFT.GigLabsQA_NFTCollectionPublic,MetadataViews.ResolverCollection}>(
    ...
    else if (signer.getCapability<&GigLabsQA_NFT.Collection{NonFungibleToken.CollectionPublic,GigLabsQA_NFT.GigLabsQA_NFTCollectionPublic,MetadataViews.ResolverCollection}>(GigLabsQA_NFT.CollectionPublicPath).borrow() == nil) {
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&GigLabsQA_NFT.Collection>(from: GigLabsQA_NFT.CollectionStoragePath)
    ...
    .getCapability(GigLabsQA_NFT.CollectionPublicPath)!
    .borrow<&{GigLabsQA_NFT.GigLabsQA_NFTCollectionPublic}>()!
  }
  pre {
