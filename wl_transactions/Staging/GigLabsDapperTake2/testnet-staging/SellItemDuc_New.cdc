...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let GigLabsDapperTake2_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &GigLabsDapperTake2_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&GigLabsDapperTake2_NFT.Collection>(from: GigLabsDapperTake2_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&GigLabsDapperTake2_NFT.Collection>(GigLabsDapperTake2_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: GigLabsDapperTake2_NFT.CollectionPublicPath)
        }

        self.GigLabsDapperTake2_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &GigLabsDapperTake2_NFT.Collection>(GigLabsDapperTake2_NFT.CollectionPublicPath)
