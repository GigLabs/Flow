...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let GigLabsDapperTake2_NFTProvider: Capability<&GigLabsDapperTake2_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(gig: AuthAccount, acct: AuthAccount) {
        ...
        let GigLabsDapperTake2_NFTCollectionProviderPrivatePath = /private/GigLabsDapperTake2_NFTCollectionProviderForNFTStorefront
        ...
        if !acct.getCapability<&GigLabsDapperTake2_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        (GigLabsDapperTake2_NFTCollectionProviderPrivatePath)!.check() {
            acct.link<&GigLabsDapperTake2_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
            (GigLabsDapperTake2_NFTCollectionProviderPrivatePath, target: GigLabsDapperTake2_NFT.CollectionStoragePath)
        }

        self.GigLabsDapperTake2_NFTProvider = acct.getCapability<&GigLabsDapperTake2_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(GigLabsDapperTake2_NFTCollectionProviderPrivatePath)!
        assert(self.GigLabsDapperTake2_NFTProvider.borrow() != nil, message: "Missing or mis-typed GigLabsDapperTake2_NFT.Collection provider")
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@GigLabsDapperTake2_NFT.NFT>(){
    ...
    pre {
        self.gigAddress == 0xd604d4601be3a3c5: "Requires valid authorizing signature"
    }
    execute {
        ...
        self.storefront.createListing(
            nftProviderCapability: self.GigLabsDapperTake2_NFTProvider,
            nftType: Type<@GigLabsDapperTake2_NFT.NFT>(),
            nftID: saleItemID,
