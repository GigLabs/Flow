...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.GigLabsDapperTake2_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &GigLabsDapperTake2_NFT.Collection>(GigLabsDapperTake2_NFT.CollectionStoragePath)
        assert(self.GigLabsDapperTake2_NFTProvider.check(), message: "Missing or mis-typed GigLabsDapperTake2_NFT.Collection provider")
