...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{GigLabsDapperTake2_NFT.GigLabsDapperTake2_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&GigLabsDapperTake2_NFT.Collection>(from: GigLabsDapperTake2_NFT.CollectionStoragePath) == nil {
        ...
        buyer.link<&GigLabsDapperTake2_NFT.Collection{GigLabsDapperTake2_NFT.GigLabsDapperTake2_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(
    ...
    else if (buyer.getCapability<&GigLabsDapperTake2_NFT.Collection{GigLabsDapperTake2_NFT.GigLabsDapperTake2_NFTCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(GigLabsDapperTake2_NFT.CollectionPublicPath).borrow() == nil) {

        // Unlink the current capability exposing the GigLabsDapperTake2_NFT collection,
        // as it needs to be replaced with an updated capability
        buyer.unlink(GigLabsDapperTake2_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&GigLabsDapperTake2_NFT.Collection>(from: GigLabsDapperTake2_NFT.CollectionStoragePath)
    ...
    .getCapability(GigLabsDapperTake2_NFT.CollectionPublicPath)!
    .borrow<&{GigLabsDapperTake2_NFT.GigLabsDapperTake2_NFTCollectionPublic}>()!
  }
  pre {
    // Make sure the seller is the right account
    self.gigAuthAccountAddress == 0xd604d4601be3a3c5 && sellerAddress == 0x6f8aa41eedff1158: "seller must be GigLabs"
  }
  execute {
