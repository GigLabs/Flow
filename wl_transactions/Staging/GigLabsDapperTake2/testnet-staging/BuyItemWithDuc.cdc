...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let GigLabsDapperTake2_NFTCollection: &GigLabsDapperTake2_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&GigLabsDapperTake2_NFT.Collection>(from: GigLabsDapperTake2_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-GigLabsDapperTake2_NFT.createEmptyCollection(nftType: Type<@GigLabsDapperTake2_NFT.NFT>()), to: GigLabsDapperTake2_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(GigLabsDapperTake2_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&GigLabsDapperTake2_NFT.Collection>(GigLabsDapperTake2_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: GigLabsDapperTake2_NFT.CollectionPublicPath)
        }
        ...
        self.GigLabsDapperTake2_NFTCollection = buyer.storage.borrow<&GigLabsDapperTake2_NFT.Collection>(
