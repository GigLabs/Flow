...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        self.CNN_INT_NFTProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>(CNN_INT_NFT.CollectionStoragePath)
        assert(self.CNN_INT_NFTProvider.check(), message: "Missing or mis-typed CNN_INT_NFT.Collection provider")
