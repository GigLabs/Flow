...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let CNN_INT_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &CNN_INT_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&CNN_INT_NFT.Collection>(from: CNN_INT_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&CNN_INT_NFT.Collection>(CNN_INT_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: CNN_INT_NFT.CollectionPublicPath)
        }

        self.CNN_INT_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &CNN_INT_NFT.Collection>(CNN_INT_NFT.CollectionPublicPath)
