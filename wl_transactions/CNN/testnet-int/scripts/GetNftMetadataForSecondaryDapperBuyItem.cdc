...
access(all) fun main(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64): PurchaseData {
    ...
    let collectionRef = acct.capabilities.borrow<&CNN_INT_NFT.Collection>(CNN_INT_NFT.CollectionPublicPath)
        ?? panic("Could not borrow collection from address")
    let nft = collectionRef.borrowCNN_INT_NFT(id: listingDetails.nftID)
        ?? panic("No item with that ID")
    let setMetadata = CNN_INT_NFT.getSetMetadata(setId: nft.setId) ?? panic("no metadata found")
