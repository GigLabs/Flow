...
transaction(storefrontAddress: Address, listingResourceID: UInt64, expectedPrice: UFix64) {
    ...
    let CNN_NFTCollection: &CNN_NFT.Collection
    ...
    prepare(
        ...
        if buyer.storage.borrow<&CNN_NFT.Collection>(from: CNN_NFT.CollectionStoragePath) == nil {

            // Create a new empty collection and save it to the account
            buyer.storage.save(<-CNN_NFT.createEmptyCollection(nftType: Type<@CNN_NFT.NFT>()), to: CNN_NFT.CollectionStoragePath)

            // create a public capability for the collection
            buyer.capabilities.unpublish(CNN_NFT.CollectionPublicPath)
            let collectionCap = buyer.capabilities.storage.issue<&CNN_NFT.Collection>(CNN_NFT.CollectionStoragePath)
            buyer.capabilities.publish(collectionCap, at: CNN_NFT.CollectionPublicPath)
        }
        ...
        self.CNN_NFTCollection = buyer.storage.borrow<&CNN_NFT.Collection>(
