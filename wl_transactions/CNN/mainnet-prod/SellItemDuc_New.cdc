...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let CNN_NFTProvider: Capability<auth(NonFungibleToken.Withdraw) &CNN_NFT.Collection>
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&CNN_NFT.Collection>(from: CNN_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&CNN_NFT.Collection>(CNN_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: CNN_NFT.CollectionPublicPath)
        }

        self.CNN_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &CNN_NFT.Collection>(CNN_NFT.CollectionPublicPath)
