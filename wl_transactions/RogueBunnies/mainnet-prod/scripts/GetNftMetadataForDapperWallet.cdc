...
pub fun main(address: Address, listingResourceID: UInt64): PurchaseData {
    ...
    let collection = account.getCapability(RogueBunnies_NFT.CollectionPublicPath)
    .borrow<&{RogueBunnies_NFT.RogueBunnies_NFTCollectionPublic}>()
    ?? panic("Could not borrow a reference to the collection")
    let nft = collection.borrowRogueBunnies_NFT(id: listingDetails.nftID) 
            ?? panic("Could not borrow a reference to the collection")
    let setMeta = RogueBunnies_NFT.getSetMetadata(setId: nft!.setId)!
