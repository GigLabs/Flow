...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    prepare(gig: &Account, acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        if acct.storage.borrow<&RogueBunnies_NFT.Collection>(from: RogueBunnies_NFT.CollectionStoragePath) == nil {
            let collectionCap = acct.capabilities.storage.issue<&RogueBunnies_NFT.Collection>(RogueBunnies_NFT.CollectionStoragePath)
            acct.capabilities.publish(collectionCap, at: RogueBunnies_NFT.CollectionPublicPath)
        }

        self.RogueBunnies_NFTProvider = acct.capabilities.get<auth(NonFungibleToken.Withdraw) &RogueBunnies_NFT.Collection>(RogueBunnies_NFT.CollectionPublicPath)
