...
transaction(sellerAddress: Address, nftIDs: [UInt64], price: UFix64, metadata: {String: String}) {
  ...
  let buyerNFTCollection: &AnyResource{Bobblz_NFT.Bobblz_NFTCollectionPublic}
  ...
  prepare(gig: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
    ...
    if buyer.borrow<&Bobblz_NFT.Collection>(from: Bobblz_NFT.CollectionStoragePath) == nil {
      ...
      buyer.link<&Bobblz_NFT.Collection{NonFungibleToken.CollectionPublic,Bobblz_NFT.Bobblz_NFTCollectionPublic,MetadataViews.ResolverCollection}>(
  ...
  // Metadata Resolver interface for the Metadata Standard views
  else if !buyer.getCapability<&Bobblz_NFT.Collection{NonFungibleToken.CollectionPublic,Bobblz_NFT.Bobblz_NFTCollectionPublic,MetadataViews.ResolverCollection}>
      (Bobblz_NFT.CollectionPublicPath)!.check() { 

      // Unlink the current capability exposing the Bobblz_NFT collection,
      // as it needs to be replaced with an updated capability
      buyer.unlink(Bobblz_NFT.CollectionPublicPath)
    ...
    // withdraw NFT
    let gigNftProvider = gig.borrow<&Bobblz_NFT.Collection>(from: Bobblz_NFT.CollectionStoragePath)
  ...
  pre {
    ...
    self.gigAuthAccountAddress == 0xd45e2bd9a3d5003b && sellerAddress == 0xb82ba4137573164c: "seller must be GigLabs"
  }
  execute {
